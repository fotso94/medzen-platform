# ASR base-model AWS change packet 2026-003B-S1 — full-suite shard 1, attempt 30

Status: **DRAFT — INDEPENDENT SELF-REVIEW AND DELEGATED APPROVAL REQUIRED — NOT EXECUTABLE**

## Required approval phrase

> Approve ASR base-model AWS change packet 2026-003B-S1 only, authorizing numbered attempt 30 for one non-transferable 10,800-second offline evaluation attempt within a fresh $10 ceiling and continuing ASR-EVAL-RUNTIME-RISK-ACCEPTANCE-2026-003 at SHA-256 43321489ce6f9f3a33f86445601b5bd65d99d2f1746747944b2a185742341034.

## Attempt-29 terminal result

Attempt 29 is consumed: the image-baked load_runtime_rows bound (1..540)
refused the 3,616-row shard at PILOT_START (journal sequence 1-2, 20ms of
container time; refusal record committed). The driver now validates the
exact bindings-bound row count and feeds suite rows through the image's
own validator in 540-row slices with cross-slice duplicate detection —
image-native per-row validation, digest unchanged, no inference-parameter
changes. MEDZEN_EXPECTED_ROWS is injected per bindings and gated in the
render verifier. The host guard is upgraded to caffeinate -ims with a
keep-lid-open advisory (lid-close sleep stretched both suite launches).
...
2. Committed stage-1 dry validation. 3. Execute attempt 30; per-shard
