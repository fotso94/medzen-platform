#!/usr/bin/env bash
# Immediate cleanup path for a separately approved 003C-D execution.
set -euo pipefail
...
[[ -f "$kubeconfig" && -f "$workload_render" && -f "$authorization" ]] || {
  echo "REFUSING: a required 003C-D cleanup file is absent" >&2
  exit 2
}
[[ "$packet_sha256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "REFUSING: exact 003C-D packet SHA-256 is required" >&2
  exit 2
}
[[ "${AWS_PROFILE:-}" == "medzen" ]] || {
...
.venv/bin/python scripts/b6a_003c_d_deadline.py disarm \
