#!/usr/bin/env bash
# Independently invocable cleanup for B6 packet 2026-018.
set -euo pipefail
...
record_refused_cleanup() {
  ...
  if [[ $original_status -ne 0 && ! -e "$receipts_dir/${cleanup_receipt_stage}.json" ]]; then
    ...
    .venv/bin/python scripts/b6_6_receipt_v2.py "$cleanup_receipt_stage" REFUSED \
      --receipts-dir "$receipts_dir" --payload "$payload" >/dev/null || true
  fi
  exit "$original_status"
}
trap record_refused_cleanup EXIT
...
[[ "$receipts_dir" == "$repo_root/platform/evidence/receipts/B6-2026-018-LIVE" ]] || { echo "REFUSING: cleanup receipt directory differs" >&2; exit 2; }
...
cleanup_plan="/private/tmp/b6-018-cleanup-$PPID.tfplan"
targets=(
...
if [[ "$change_count" != "0" ]]; then
  if [[ -e "$receipts_dir/terraform_window.json" ]] \
    && jq -e '.status == "PASS"' "$receipts_dir/terraform_window.json" >/dev/null 2>&1; then
    .venv/bin/python scripts/check_b6_6_images_before_endpoints_plan.py destroy "$cleanup_plan"
  else
    .venv/bin/python scripts/check_b6_6_images_before_endpoints_plan.py cleanup "$cleanup_plan"
  fi
...
if [[ -d "$credential_receipts" && ! -e "$credential_receipts/cleanup.json" ]]; then
  .venv/bin/python scripts/b6_6_images_before_endpoints_credential_stage.py cleanup \
...
if [[ -d "$receipts_dir" ]]; then
  .venv/bin/python scripts/b6_6_receipt_v2.py "$cleanup_receipt_stage" PASS \
