#!/usr/bin/env bash
# Execute only independently reviewed and owner-approved packet 2026-017.
set -euo pipefail
...
receipts_dir="$4"
token_file="$5"
expected_receipts="$repo_root/platform/evidence/receipts/B6-2026-017-LIVE"
...
[[ -f "$kubeconfig" && -f "$authorization" ]] || { echo "REFUSING: required B6.6 execution input is absent" >&2; exit 2; }
[[ "$token_file" == "/private/tmp/medzen-b6-6-client-token" ]] || { echo "REFUSING: exact synthetic token path is required" >&2; exit 2; }
[[ ! -e "$token_file" ]] || { echo "REFUSING: stage 0 requires no pre-existing local synthetic material" >&2; exit 2; }
[[ "$receipts_dir" == "$expected_receipts" ]] || { echo "REFUSING: exact packet-2026-017 receipt directory is required" >&2; exit 2; }
[[ ! -e "$receipts_dir" ]] || { echo "REFUSING: packet-2026-017 receipt directory already exists" >&2; exit 2; }
...
from scripts.b6_6_successor_bindings import validate
validate(Path(sys.argv[1]), sys.argv[2], Path(sys.argv[3]))
PY

# Credential restoration is the packet's compute-free stage 0. Its own trap
# restores zero state if any substage refuses. No deadline or worker starts yet.
bash scripts/b6_6_successor_credential_stage.sh \
...
cleanup() {
  ...
  bash scripts/b6_6_successor_cleanup.sh "$kubeconfig" "$authorization" "$packet_sha256" "$receipts_dir" "$token_file"
  cleanup_status=$?
  if [[ $cleanup_status -ne 0 ]]; then
    echo "REFUSING: B6.6 successor cleanup proof failed; AWS deadlines remain armed" >&2
    exit "$cleanup_status"
  fi
...
credential_payload="$(.venv/bin/python scripts/b6_6_successor_secret_preflight.py \
...
local_payload="$(jq -nc --argjson credential "$credential_payload" '{authorization_record:"HASH_BOUND_OWNER_APPROVED",cost_registry:"COST-REGISTRY-2026-004",source_bindings_verified:true,credential_stage:"PASS_VERIFIED_FRESH",credential_version_id:$credential.fresh_version_id,credential_material_sha256:$credential.fresh_material_sha256,credential_value_sha256:$credential.secret_value_sha256,prior_credential_versions_unstaged:($credential.prior_version_count==3)}')"
.venv/bin/python scripts/b6_6_receipt.py local_bindings PASS --receipts-dir "$receipts_dir" --payload "$local_payload"

# Stage 1: independent AWS-side worker shutdown exists before capacity rises.
deadline_payload="$(.venv/bin/python scripts/b6_6_successor_deadline.py arm)"
...
.venv/bin/python scripts/b6_6_successor_secret_preflight.py --verification-receipt "$receipts_dir/credential/verification.json" --profile medzen >/dev/null
...
window_plan="/private/tmp/b6-017-create-$PPID.tfplan"
targets=(
...
.venv/bin/python scripts/check_b6_6_successor_window_plan.py create "$window_plan"
...
endpoint_payload="$(.venv/bin/python scripts/b6_6_successor_probe_endpoints.py available --profile medzen --wait-seconds 900)"
...
fargate_payload=""
if fargate_payload="$(.venv/bin/python scripts/b6_6_successor_fargate_probe.py --target-url "http://$alb_hostname/readyz" --profile medzen --wait-seconds 600)"; then
