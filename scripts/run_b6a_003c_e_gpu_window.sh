#!/usr/bin/env bash
# Execute only a separately approved, independently IAM-reviewed 003C-E packet.
set -euo pipefail
...
.venv/bin/python scripts/b6a_003c_e_bindings.py \
...
cleanup() {
  ...
  trap - EXIT INT TERM
  set +e
  scripts/b6a_003c_e_cleanup.sh \
    "$kubeconfig" "$workload_render" "$authorization" "$packet_sha256" "$receipts_dir"
  cleanup_status=$?
  if [[ $cleanup_status -ne 0 ]]; then
    echo "REFUSING: cleanup proof failed; 003C-E deadline remains armed" >&2
    exit "$cleanup_status"
  fi
...
.venv/bin/python scripts/b6a_003c_e_deadline.py arm \
  --authorization "$authorization" \
  --packet-sha256 "$packet_sha256" \
  --receipts-dir "$receipts_dir" \
  --window-seconds 5109
.venv/bin/python scripts/b6a_003c_e_deadline.py verify \
  --authorization "$authorization" \
  --packet-sha256 "$packet_sha256" \
  --receipts-dir "$receipts_dir"
...
.venv/bin/python scripts/run_b6a_003c_e_sampler_self_test.py \
