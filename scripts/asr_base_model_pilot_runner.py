...
def _execute_attempt(ops: Operations, context: AttemptContext) -> dict[str, Any]:
    if context.attempt not in set(range(1, 19)) or context.deadline_seconds != 10800:
        raise OperationRefusal("ATTEMPT_BOUNDARY_DIFFERS", "only attempts 1 through 18 at 10800 seconds are permitted")
