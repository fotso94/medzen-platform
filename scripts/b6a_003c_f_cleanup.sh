#!/usr/bin/env bash
# Immediate cleanup path for a separately approved 003C-F execution.
set -euo pipefail
...
[[ -f "$kubeconfig" && -f "$workload_render" && -f "$authorization" ]] || {
  echo "REFUSING: a required 003C-F cleanup file is absent" >&2
  exit 2
}
[[ "$packet_sha256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "REFUSING: exact 003C-F packet SHA-256 is required" >&2
  exit 2
}
[[ "${AWS_PROFILE:-}" == "medzen" ]] || {
...
.venv/bin/python scripts/b6a_003c_f_deadline.py disarm \
