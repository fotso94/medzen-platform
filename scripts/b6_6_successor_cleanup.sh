#!/usr/bin/env bash
# Independently invocable cleanup for B6-AWS-CHANGE-PACKET-2026-017.
set -euo pipefail
...
receipts_dir="$4"
token_file="$5"
credential_receipts="$receipts_dir/credential"
...
# Stop the one permitted probe before deleting its temporary cluster.
...
# The Ingress is removed while controller and workers remain available so the
# controller-owned ALB can be observed absent before its own teardown.
...
cleanup_plan="/private/tmp/b6-017-cleanup-$PPID.tfplan"
targets=(
...
if [[ "$change_count" != "0" ]]; then
  if [[ -e "$receipts_dir/terraform_window.json" ]]; then
    .venv/bin/python scripts/check_b6_6_successor_window_plan.py destroy "$cleanup_plan"
  else
    .venv/bin/python scripts/check_b6_6_successor_window_plan.py cleanup "$cleanup_plan"
  fi
...
# The stage-0 cleanup writes its own durable receipt and never force-deletes.
cleanup_step="credential_stage_cleanup"
if [[ -d "$credential_receipts" && ! -e "$credential_receipts/cleanup.json" ]]; then
...
cleanup_step="deadline_disarm"
.venv/bin/python scripts/b6_6_successor_deadline.py disarm --wait-seconds 1800
...
if [[ -e "$receipts_dir/deadline.json" ]]; then
  .venv/bin/python scripts/b6_6_receipt.py "$cleanup_receipt_stage" PASS --receipts-dir "$receipts_dir" --payload '{"alb_count":0,"approved_asr_changes":0,"cpu_asg_instances":0,"cpu_desired":0,"deadline_actions":0,"deployments":0,"endpoint_security_groups":0,"gpu_asg_instances":0,"gpu_desired":0,"ingresses":0,"local_material_removed":true,"probe_vpc_endpoints":0,"production_ssm_pointer_changes":0,"synthetic_secret":"SCHEDULED_RECOVERABLE_DELETION","window_terraform_resources":0}'
else
  # Before the main receipt chain starts, the credential cleanup receipt is
  # the durable proof. A main cleanup receipt would correctly refuse because
  # its immutable schema requires a prior deadline receipt.
  [[ -e "$credential_receipts/cleanup.json" ]]
  echo "PASS_B6_6_SUCCESSOR_PRE_DEADLINE_ZERO_STATE"
fi
