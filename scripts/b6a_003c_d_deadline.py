#!/usr/bin/env python3
"""Arm the 003C-D deadline within the confirmed remaining GPU allowance."""
...
ACTION_NAME = "medzen-b6a-003c-d-deadline-scale-zero"
...
class DeadlineControl(base.DeadlineControl):
    def arm(self, *, now: datetime, window_seconds: int) -> dict[str, Any]:
        ...
        if not MIN_WINDOW_SECONDS <= window_seconds <= MAX_WINDOW_SECONDS:
            raise base.DeadlineRefusal("003C-D window must be between 300 and 6520 seconds")
        ...
        if len(actions) != 1 or actions[0].get("ScheduledActionName") != ACTION_NAME:
            raise base.DeadlineRefusal("003C-D deadline could not be read back exactly")
        ...
        return {
            ...
            "conservative_prior_billed_seconds": 680,
    ...
    def verify(self, *, now: datetime) -> dict[str, Any]:
        ...
        actions = self._actions()
        if len(actions) != 1 or actions[0].get("ScheduledActionName") != ACTION_NAME:
            raise base.DeadlineRefusal("exact 003C-D deadline action is absent")
        deadline = base._utc(actions[0]["StartTime"])
        if deadline <= now or deadline > now + timedelta(seconds=MAX_WINDOW_SECONDS):
            raise base.DeadlineRefusal("003C-D deadline is expired or exceeds 6520 seconds")
        self._verify_action(actions[0], deadline)
        return {"status": "PASS", "deadline_utc": deadline.isoformat().replace("+00:00", "Z")}

    def disarm_003c_d_after_zero(self, *, kubeconfig: Path) -> dict[str, Any]:
        ...
        actions = self._actions()
        if len(actions) != 1 or actions[0].get("ScheduledActionName") != ACTION_NAME:
            raise base.DeadlineRefusal("003C-D deadline is absent or ambiguous")
        self.autoscaling.delete_scheduled_action(
            AutoScalingGroupName=base.ASG_NAME,
            ScheduledActionName=ACTION_NAME,
        )
        if self._actions():
            raise base.DeadlineRefusal("003C-D deadline remains after deletion")
        return {
            "status": "PASS",
            "action": ACTION_NAME,
...
def main() -> int:
    ...
    try:
        ...
        if args.mode == "arm":
            ...
            while True:
                try:
                    result = control.disarm_003c_d_after_zero(kubeconfig=args.kubeconfig)
                    break
