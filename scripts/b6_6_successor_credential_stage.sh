#!/usr/bin/env bash
# In-packet stage 0: restore and rotate synthetic credentials before compute.
set -euo pipefail
...
expected_root="$repo_root/platform/evidence/receipts/B6-2026-017-LIVE"
...
[[ "$receipts_root" == "$expected_root" ]] || { echo "REFUSING: exact successor receipt directory is required" >&2; exit 2; }
[[ ! -e "$receipts_root" ]] || { echo "REFUSING: successor receipt directory already exists" >&2; exit 2; }
...
plan_prefix="/private/tmp/b6-017-credential-$PPID"
mutation_started=0
...
cleanup_after_refusal() {
  ...
  if [[ ! -e "$credential_receipts/cleanup.json" ]]; then
    .venv/bin/python scripts/b6_6_successor_credential_stage.py cleanup \
      --authorization "$authorization" --packet-sha256 "$packet_sha256" \
      --receipts-dir "$credential_receipts" --apply
    cleanup_status=$?
  fi
  if [[ $cleanup_status -ne 0 ]]; then
    echo "REFUSING: successor stage-0 cleanup proof failed" >&2
    exit 70
  fi
  exit "$original_status"
}
...
# Stage 0.1: exact pending secret and zero-compute preflight.
.venv/bin/python scripts/b6_6_successor_credential_stage.py preflight \
  --authorization "$authorization" --packet-sha256 "$packet_sha256" \
  --receipts-dir "$credential_receipts" --apply

# Stage 0.2: restore the exact ARN without reading old plaintext.
mutation_started=1
.venv/bin/python scripts/b6_6_successor_credential_stage.py restore \
  --authorization "$authorization" --packet-sha256 "$packet_sha256" \
  --receipts-dir "$credential_receipts" --apply

# Stage 0.3: import only the restored secret into its reviewed address.
...
.venv/bin/python scripts/b6_6_successor_credential_stage.py record-terraform-import \
  --authorization "$authorization" --packet-sha256 "$packet_sha256" \
  --receipts-dir "$credential_receipts" --payload-json "$import_payload" --apply

# Stage 0.4: normalize only the imported Terraform representation if required.
...
.venv/bin/python scripts/b6_6_successor_credential_stage.py record-terraform-normalization \
  --authorization "$authorization" --packet-sha256 "$packet_sha256" \
  --receipts-dir "$credential_receipts" --payload-json "$normalization_payload" --apply

# Stage 0.5: restore the unchanged reader and KMS boundaries.
...
.venv/bin/python scripts/b6_6_successor_credential_stage.py record-terraform-reconciliation \
  --authorization "$authorization" --packet-sha256 "$packet_sha256" \
  --receipts-dir "$credential_receipts" --payload-json "$reconciliation_payload" --apply

# Stage 0.6 and 0.7: generate fresh material, publish and verify it.
.venv/bin/python scripts/b6_6_successor_credential_stage.py rotate \
  --authorization "$authorization" --packet-sha256 "$packet_sha256" \
  --receipts-dir "$credential_receipts" --apply
.venv/bin/python scripts/b6_6_successor_credential_stage.py verify \
  --authorization "$authorization" --packet-sha256 "$packet_sha256" \
  --receipts-dir "$credential_receipts" --apply
...
mutation_started=0
trap - EXIT INT TERM
echo "PASS_B6_6_SUCCESSOR_CREDENTIAL_STAGE_0"
