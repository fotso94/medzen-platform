#!/usr/bin/env python3
"""Verify packet-2026-018 dynamic stage-0 credentials before compute."""
...
from scripts.b6_6_images_before_endpoints_credential_stage import (
    CURRENT_VERSION,
    LEGACY_VERSIONS,
)
from scripts.b6_6_token_binding import TOKEN_PATH
...
def verify(secret_client, iam_client, verification_receipt: Path) -> dict:
    ...
    if (
        ...
        or value.get("authorization_id") != "B6-AWS-AUTH-2026-018"
    ...
    expected = {fresh: ["AWSCURRENT"], CURRENT_VERSION: []}
    expected.update({item: [] for item in LEGACY_VERSIONS})
