#!/usr/bin/env bash
# Immediate cleanup path for a separately approved 003C-C retry.
set -euo pipefail
...
[[ "$packet_sha256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "REFUSING: exact 003C-C packet SHA-256 is required" >&2
  exit 2
}
[[ "${AWS_PROFILE:-}" == "medzen" ]] || {
...
.venv/bin/python scripts/b6a_003c_c_deadline.py disarm \
