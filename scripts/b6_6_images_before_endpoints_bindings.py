#!/usr/bin/env python3
"""Validate review, packet and source bindings for B6 packet 2026-018."""
...
AUTH_ID = "B6-AWS-AUTH-2026-018"
PACKET_ID = "B6-AWS-CHANGE-PACKET-2026-018"
MANIFEST_PATH = "platform/manifests/B6-CLIENT-API-KEYS-RESTORE-2026-005.json"
REQUIRED_SOURCES = {
    ...
    "infra/eks.tf",
    "infra/variables.tf",
    "pipeline/b6_integration_receipts_v2.py",
    MANIFEST_PATH,
    ...
    "scripts/b6_6_images_before_endpoints_bindings.py",
    "scripts/b6_6_images_before_endpoints_cleanup.sh",
    "scripts/b6_6_images_before_endpoints_credential_stage.py",
    "scripts/b6_6_images_before_endpoints_credential_stage.sh",
    "scripts/b6_6_images_before_endpoints_secret_preflight.py",
    "scripts/b6_6_images_before_endpoints_window.sh",
    ...
    "scripts/b6_6_receipt_v2.py",
    "scripts/b6_6_stage_runtime.sh",
    ...
    "scripts/check_b6_6_images_before_endpoints_plan.py",
    ...
    "tests/test_b6_6_images_before_endpoints.py",
}
...
def validate(path: Path, packet_sha256: str, root: Path) -> dict[str, Any]:
    if re.fullmatch(r"[0-9a-f]{64}", packet_sha256) is None:
        raise BindingRefusal("exact packet-2026-018 SHA-256 is required")
    try:
        value = json.loads(path.read_bytes())
    except Exception as exc:
        raise BindingRefusal("packet-2026-018 authorization is absent or unreadable") from exc
    if value.get("id") != AUTH_ID or value.get("status") != "owner-approved":
        raise BindingRefusal("packet 2026-018 is not owner-approved")
    if value.get("packet") != {"id": PACKET_ID, "sha256": packet_sha256}:
        raise BindingRefusal("packet-2026-018 binding differs")
    ...
    if (
        ...
        raise BindingRefusal("independent packet-2026-018 review is absent")
    if value.get("credential_stage") != {
        ...
        "prior_current_version_id": "201f9790-72c4-45f7-a05b-967551532aef",
        "prior_version_count": 4,
        "plaintext_reuse_permitted": False,
    }:
        raise BindingRefusal("packet-2026-018 credential-stage binding differs")
    if value.get("cost") != {
        "registry_id": "COST-REGISTRY-2026-004",
        "allocation_id": "B6-INTEGRATION-WINDOW-2026-001",
        "maximum_usd": 10.0,
        "cumulative_seconds_before": 7548,
        "remaining_seconds_before": 6852,
        "maximum_worker_window_seconds": 4500,
        "remaining_seconds_after_full_cap": 2352,
        "retry_after_pre_proof_stop_requires_owner_allowance_decision": True,
        "new_reservation_usd": 0.0,
    }:
        raise BindingRefusal("packet-2026-018 cost binding differs")
    sources = value.get("source_bindings")
    if not isinstance(sources, dict) or set(sources) != REQUIRED_SOURCES:
        raise BindingRefusal("packet-2026-018 source binding set differs")
    for relative, expected in sorted(sources.items()):
        if relative.startswith("/") or ".." in Path(relative).parts:
            raise BindingRefusal("packet-2026-018 source path is unsafe")
        ...
        if (
            ...
            raise BindingRefusal(f"packet-2026-018 source hash differs: {relative}")
    return value
