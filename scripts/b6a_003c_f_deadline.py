#!/usr/bin/env python3
"""Arm the 003C-F deadline within the remaining GPU allowance."""
...
ACTION_NAME = "medzen-b6a-003c-f-deadline-scale-zero"
...
class DeadlineControl(base.DeadlineControl):
    def arm(self, *, now: datetime, window_seconds: int) -> dict[str, Any]:
        ...
        if not MIN_WINDOW_SECONDS <= window_seconds <= MAX_WINDOW_SECONDS:
            raise base.DeadlineRefusal("003C-F window must be between 300 and 4610 seconds")
        ...
        if len(actions) != 1 or actions[0].get("ScheduledActionName") != ACTION_NAME:
            raise base.DeadlineRefusal("003C-F deadline could not be read back exactly")
        ...
        return {
            ...
            "conservative_prior_billed_seconds": 2590,
    ...
    def verify(self, *, now: datetime) -> dict[str, Any]:
        ...
        actions = self._actions()
        if len(actions) != 1 or actions[0].get("ScheduledActionName") != ACTION_NAME:
            raise base.DeadlineRefusal("exact 003C-F deadline action is absent")
        deadline = base._utc(actions[0]["StartTime"])
        if deadline <= now or deadline > now + timedelta(seconds=MAX_WINDOW_SECONDS):
            raise base.DeadlineRefusal("003C-F deadline is expired or exceeds 4610 seconds")
        self._verify_action(actions[0], deadline)
        return {"status": "PASS", "deadline_utc": deadline.isoformat().replace("+00:00", "Z")}

    def disarm_003c_f_after_zero(self, *, kubeconfig: Path) -> dict[str, Any]:
        ...
        actions = self._actions()
        if len(actions) != 1 or actions[0].get("ScheduledActionName") != ACTION_NAME:
            raise base.DeadlineRefusal("003C-F deadline is absent or ambiguous")
        self.autoscaling.delete_scheduled_action(
            AutoScalingGroupName=base.ASG_NAME,
            ScheduledActionName=ACTION_NAME,
        )
        if self._actions():
            raise base.DeadlineRefusal("003C-F deadline remains after deletion")
        return {
            "status": "PASS",
            "action": ACTION_NAME,
...
def main() -> int:
    ...
    try:
        ...
        if args.mode == "arm":
            ...
            while True:
                try:
                    result = control.disarm_003c_f_after_zero(kubeconfig=args.kubeconfig)
                    break
