...
# The temporary SG is attached to both the Fargate task ENI and the ECR
# interface endpoints. Permit only the task's outbound TLS connection to those
# same-SG endpoints. Return traffic is stateful.
resource "aws_vpc_security_group_egress_rule" "b6_probe_to_ecr_endpoints" {
  count = local.b6_probe_resources_enabled ? 1 : 0
  ...
  from_port                    = 443
  to_port                      = 443
  description                  = "TLS from the isolated probe to its ECR interface endpoints"

  tags = local.b6_window_tags
}

# ECR layer downloads are redirected to S3. Bind that second TLS path to the
# exact prefix list generated by the packet-created S3 gateway endpoint.
resource "aws_vpc_security_group_egress_rule" "b6_probe_to_s3" {
  count = local.b6_probe_resources_enabled ? 1 : 0

  security_group_id = aws_security_group.b6_probe_endpoints[0].id
  prefix_list_id    = aws_vpc_endpoint.b6_probe_s3[0].prefix_list_id
  ip_protocol       = "tcp"
  from_port         = 443
  to_port           = 443
...
# No DNS egress rule is possible or required: security groups cannot filter
# traffic to the Amazon-provided VPC resolver (VPC CIDR base plus two).
