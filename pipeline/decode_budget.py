"""Fresh bounded ledger for PLAN-2026-004 only."""
...
CAMPAIGN = "b4-amharic-decode-compatibility"
LEDGER_KEY = f"candidates/budget/{CAMPAIGN}/ledger.json"
RATES = {"g6.xlarge": 1.0064, "c6i.2xlarge": 0.34}
WATCHDOG_S = {"builder": 1800, "decode_compatibility": 5400}
STAGE_INSTANCE = {
    "builder": "c6i.2xlarge",
    "decode_compatibility": "g6.xlarge",
}
...
def load(cli) -> tuple[dict, str | None]:
    ...
    try:
        ...
        if (ledger.get("campaign") != CAMPAIGN
            ...
            raise SystemExit("REFUSING: decode ledger identity changed")
        ...
        if code in ("NoSuchKey", "404", "NotFound"):
            return _empty(), None
        raise SystemExit(
            f"REFUSING: decode ledger is unreadable ({code})") from exc
...
def _put(cli, ledger: dict, etag: str | None) -> None:
    ...
    try:
        ...
        if exc.response.get("Error", {}).get("Code") in (
            ...
            raise SystemExit(
                "REFUSING: decode ledger changed concurrently") from exc
        raise
    back, _ = load(cli)
    if back != ledger:
        raise SystemExit("REFUSING: decode ledger readback differs")
...
def reserve(cli, stage: str, attempt: str) -> dict:
    ...
    if stale:
        raise SystemExit(
            f"REFUSING: unresolved decode reservation(s) {stale}")
    worst = worst_case_usd(stage)
    if committed_usd(ledger) + worst > CEILING_USD:
        raise SystemExit(f"REFUSING: {stage} would exceed decode ceiling")
    ledger["reservations"][rid] = {
...
def reconcile(cli, stage: str, attempt: str, actual_seconds: float,
    ...
    if reservation is None:
        raise SystemExit("REFUSING: decode instance had no reservation")
    if reservation["state"] == "reconciled":
