"""Prospective v2 receipts with bounded pre-artifact raw-output support."""
...
POLICY_V2_PATH = "platform/runtime-receipt-policy-v2.yaml"
SUPPORTED_POLICY_PATHS = {POLICY_PATH, POLICY_V2_PATH}
PRE_ARTIFACT_RAW_OUTPUT_STAGES = {
    "local_bindings",
...
MAX_PRE_ARTIFACT_OUTPUT_BYTES = 32768
TERMINAL_STATUSES = {
    "PASS",
    "REFUSED",
...
class ReceiptStore:
    ...
    def __init__(self, directory: Path, *, policy_path: Path):
        ...
        policy_reference = f"platform/{policy_path.name}"
        if policy_reference not in SUPPORTED_POLICY_PATHS:
            raise ReceiptRefusal("runtime receipt policy is unsupported")
    ...
    def load(self, stage: str) -> dict[str, Any]:
        ...
        if policy != {"path": self.policy_reference, "sha256": self.policy_sha256}:
    ...
    def persist(
        ...
        *,
        ...
        raw_fields = {key for key in ("raw_stdout", "raw_stderr") if key in payload}
        if raw_fields:
            if self.policy_reference != POLICY_V2_PATH:
                raise ReceiptRefusal("raw output requires runtime receipt policy v2")
            if stage not in PRE_ARTIFACT_RAW_OUTPUT_STAGES:
                raise ReceiptRefusal("raw output is prohibited after pre-artifact stages")
            if payload.get("pre_artifact_facts") != PRE_ARTIFACT_FACTS:
                raise ReceiptRefusal("pre-artifact raw output facts are absent")
            if not isinstance(payload.get("command_path"), str) or not payload[
                "command_path"
            ]:
                raise ReceiptRefusal("pre-artifact raw output command path is absent")
            if re.fullmatch(r"[0-9a-f]{64}", str(payload.get("command_sha256"))) is None:
                raise ReceiptRefusal("pre-artifact raw output command hash is absent")
            for key in raw_fields:
                value = payload[key]
                if not isinstance(value, str):
                    raise ReceiptRefusal(f"{key} must be text")
                if len(value.encode()) > MAX_PRE_ARTIFACT_OUTPUT_BYTES:
                    raise ReceiptRefusal(f"{key} exceeds the bounded output limit")
        ...
        receipt = {
            ...
            "policy": {"path": self.policy_reference, "sha256": self.policy_sha256},
