...
from check_b6a_003b_identity_plan import validate_plan  # noqa: E402
...
def _plan():
    trust = (
        ROOT / "platform/iam/b6a/medzen-b6a-asr-role.trust.template.json"
    ).read_text()
    policy = (
        ROOT / "platform/iam/b6a/medzen-b6a-asr-role.policy.template.json"
    ).read_text()
    return {
        "resource_changes": [
...
def test_exact_003b_identity_plan_passes():
    result = validate_plan(_plan())
    assert result["status"] == "PASS_EXACT_B6A_PACKET_2026_003B_IDENTITY_PHASE"
    assert (result["add"], result["change"], result["destroy"]) == (3, 0, 0)


def test_policy_drift_or_extra_change_refuses():
    plan = _plan()
